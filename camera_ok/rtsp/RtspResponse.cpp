...
RtspResponse::RtspResponse(int s, sockaddr_in bindAddr, sockaddr_in connectAddr)
{
	m_Socket = s;
	m_BindAddr = bindAddr;
									...
									INT32 ssrc)
{
	...
	snprintf(temp, 100, "server_port=%u-%u", serverRtpPort, serverRtpPort+1);
	server_port = temp;

	snprintf(temp, 100, "client_port=%u-%u", targetRtpPort, targetRtpPort+1);
	client_port = temp;

	snprintf(temp, 100, "ssrc=%u", ssrc);
	ssrc_ = temp;
...
//////////////trj added for g++///g++ transform() parameter 3 must be function object ,no function pointer/////
class ToLower
{
	public:
...
BOOL RtspResponse::SendResponse(PCSTR responseType)
{
	...
	snprintf(cseq, 256, "CSeq: %u", m_CSeq);
	snprintf(session, 256, "Session: %I64u", m_Session);
