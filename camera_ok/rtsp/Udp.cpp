...
//	i_val = 1;	// 非阻塞方式
	...
	error = fcntl(m_Socket, F_SETFL,flags | O_NONBLOCK);
	...
	socklen_t len;
	error = getsockname(m_Socket, (sockaddr*)&m_BindAddr, &len);
	if (error == SOCKET_ERROR)
	{
		ReportError();
		return FALSE;
...
BOOL Udp::Connect(string connectIp, int connectPort)
{
	...
	printf("trj test RTP/UDP connect port = %d , ip = %s kkkkkkkkkkkkkkkkkkkkkkkkk\n",connectPort,connectIp.c_str());
